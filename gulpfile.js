...
gulp.task('distPlugins', function () {
    ...
    return merge(jsPlugins, pluginCss);
});

/* projectAi css/js */
gulp.task('paAssets', function () {
    ...
    var martCss = gulp.src('src/style.css')
        .pipe(cleancss())
        .pipe(gulp.dest('dist'));
    var martHtml = gulp.src('./src/*.html', { read: false })
        .pipe(gulp.dest('dist'));
...
gulp.task('dist', ['sass', 'distPlugins', 'paAssets', 'distFonts'], function () {
    gulp.src('./dist/*.html')
        .pipe(
